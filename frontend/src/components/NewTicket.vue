...
export default {
  ...
  computed: {
    isFormValid() {
      return !(this.formData.object && this.formData.description && this.formData.priority && this.formData.status);
    }
  }
      ...
      <div class="col">
        <form @submit.prevent="saveTicket()" action="/">
          <div class="form-floatingmb-2">
            ...
            <input required="required" v-model="formData.object" ref="objectField" class="form-control" type="text">
          </div>
          <div class="form-floating mb-2">
            <span>Descrizione problema</span>
            <textarea required="required" v-model="formData.description" ref="descriptionField" class="form-control"
              placeholder="Leave a comment here" id="floatingTextarea"></textarea>
          </div>
          <span>Priorità</span>
          <select required="required" v-model="formData.priority" ref="priorityField" class="form-select mt-0 mb-2"
          ...
          <span>Stato</span>
          <select required="required" v-model="formData.status" ref="statusField" class="form-select mt-0"
            aria-label="Default select example">
          ...
          <button type="submit" class="btn btn-primary mt-2" data-bs-toggle="modal" data-bs-target="#staticBackdrop"
            :disabled="isFormValid">Salva</button>
